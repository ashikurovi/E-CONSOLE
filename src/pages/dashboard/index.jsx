...
const SectionCard = ({ title, children, className = "" }) => (
  <div
    className={`bg-white dark:bg-[#1a1f26] border border-gray-100 dark:border-gray-800 rounded-xl p-5 shadow-sm ${className}`}
  >
  ...
  <div className="bg-white dark:bg-[#1a1f26] bg p-4 rounded-xl border border-gray-100 dark:border-gray-800 flex items-center justify-between shadow-sm relative overflow-hidden">
    <div>
      ...
      <div className="flex items-baseline gap-2">
        ...
        {trend && (
          <span className="text-xs font-medium text-emerald-500 bg-emerald-50 dark:bg-emerald-500/10 px-1.5 py-0.5 rounded-full">
            {trend}
          </span>
        )}
      </div>
      <Link
        to="#"
        className="text-xs text-purple-600 font-medium mt-2 inline-block hover:underline"
      >
        View All
      </Link>
    </div>
...
const DashboardPage = () => {
  ...
  const revenue =
    stats.find(
      (s) => s.title?.toLowerCase().includes("revenue") || s.tone === "green",
    )?.value || "$0.00";
  const ordersCount =
    stats.find(
      (s) =>
        s.title?.toLowerCase().includes("order") && !s.title.includes("Value"),
    )?.value || "0";
  const customersCount =
    stats.find((s) => s.title?.toLowerCase().includes("customer"))?.value ||
    "0";
    ...
    <div className="min-h-screen bg-[#f7f8f9] dark:bg-[#0b0f14] p-4 lg:p-8 space-y-6">
      ...
      <div className="rounded-2xl bg-gradient-to-r from-[#7c3aed] to-[#9333ea] text-white p-8 relative overflow-hidden shadow-lg">
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="text-white/80 mb-6">
            You have 15+ invoices saved to draft that require attention.
          </p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div className="xl:col-span-3 space-y-4">
          ...
          <Button
            variant="ghost"
            className="w-full text-purple-600 hover:text-purple-700 hover:bg-purple-50 justify-between group"
          >
            View All{" "}
        ...
        <div className="xl:col-span-3 bg-white dark:bg-[#1a1f26] border border-gray-100 dark:border-gray-800 rounded-xl p-5 flex flex-col">
          ...
          <Button
            variant="outline"
            className="w-full mt-4 border-gray-100 dark:border-gray-700"
          >
            All Customers
          </Button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-[#1a1f26] border border-gray-100 dark:border-gray-800 rounded-xl p-5">
          ...
          <div className="space-y-1">
            ...
            {[1, 2].map((i) => (
              <div
                key={i}
                className="flex items-center justify-between p-2 hover:bg-gray-50 dark:hover:bg-white/5 rounded-lg"
              >
                <div className="flex items-center gap-3">
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={i} className="flex items-center justify-between">
                  ...
                  <Badge
                    className={`bg-${q.tone}-50 text-${q.tone}-600 border-${q.tone}-100 hover:bg-${q.tone}-100`}
                  >
                    {q.status}
                  </Badge>
                </div>
        ...
        <div className="bg-white dark:bg-[#1a1f26] border border-gray-100 dark:border-gray-800 rounded-xl p-5 flex flex-col items-center justify-center">
          <h3 className="font-semibold text-lg text-gray-900 dark:text-white mb-2 self-start">
            Top Sales Statistics
          </h3>
          <div className="flex gap-2 text-xs mb-4 w-full">
            <div className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-[#7c3aed]"></span> Dell
              XPS 13
            </div>
            <div className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-[#ec4899]"></span> Nike
              T-shirt
            </div>
          </div>
          <TopSalesChart />
