  ...
  return (
    <div className="rounded-2xl bg-white dark:bg-[#242424] border border-black/10 dark:border-white/10 p-4">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold">{t("termsConditions.title")}</h2>
        {latestTerms ? (
          <Button
            size="sm"
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="text-sm text-gray-500 dark:text-gray-400">
            <p>{t("termsConditions.lastUpdated")}: {latestTerms.updatedAt ? new Date(latestTerms.updatedAt).toLocaleString() : "-"}</p>
            <p>{t("termsConditions.created")}: {latestTerms.createdAt ? new Date(latestTerms.createdAt).toLocaleString() : "-"}</p>
          </div>
        </div>
      ) : (
        <div className="text-center py-8 text-gray-500">
          <p className="mb-4">{t("termsConditions.noPolicyFound")}</p>
        </div>
      )}
