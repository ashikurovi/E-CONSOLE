          ...
          <div className="space-y-4 text-sm">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="border border-black/10 dark:border-white/10 rounded-lg p-3 bg-black/5 dark:bg-white/5 space-y-2">
                  <div className="flex justify-between">
                    <span className="text-xs text-black/60 dark:text-white/60">Payment Status:</span>
                    <span className="text-xs font-medium">{user.paymentInfo?.paymentstatus ?? "-"}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-xs text-black/60 dark:text-white/60">Payment Method:</span>
                    <span className="text-xs font-medium">{user.paymentInfo?.paymentmethod ?? "-"}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-xs text-black/60 dark:text-white/60">Amount:</span>
                    <span className="text-xs font-medium">
                      {user.paymentInfo?.amount ? `$${user.paymentInfo.amount.toFixed(2)}` : "-"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-xs text-black/60 dark:text-white/60">Package Name:</span>
                    <span className="text-xs font-medium">{user.paymentInfo?.packagename ?? "-"}</span>
                  </div>
                </div>
              </div>
